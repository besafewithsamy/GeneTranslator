  ...
  return (
    <div className="group relative">
      ...
      >
        ...
        <span className={`mt-0.5 block text-[11px] ${item.type === 'stop' ? 'text-rose-600 dark:text-rose-300' : item.type === 'start' ? 'text-emerald-700 dark:text-emerald-300' : 'text-sky-700 dark:text-sky-300'}`}>
          {item.abbr}
        </span>
      </button>
      <div className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden w-44 -translate-x-1/2 rounded-lg border border-zinc-300 bg-white p-3 text-left shadow-xl group-hover:block group-focus-within:block dark:border-white/10 dark:bg-[#12141c]">
        ...
        <p className="mt-1 text-[11px] font-medium uppercase tracking-wider text-zinc-500">{t('codon.tooltip.type', { type: typeLabel })}</p>
      </div>
    </div>
  );
}
